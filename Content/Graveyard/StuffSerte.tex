...
We will use the following notation. The description will closely follow Subsection 3.2 in \cite{conchon--kerjanStableGraphMetric2020}. Write $D(\R_+,\R_+)$ for the space of c\`adl\`ag functions $f:\R_+\to \R_+$. We will write $\cE$ for the space of excursions, i.e.
...
\myworries{Results like in Subsection 3.2 in \cite{conchon--kerjanStableGraphMetric2020} follow from the fact that the limiting process is of the form $(c_1 B_t-c_2 t^2,t\geq 0)$ and results on excursions of spanning forest of $G(n,1/n)$}
% We will use the following facts about $(R_t,t\geq 0)$, that are proved in \cite{conchon--kerjanStableGraphMetric2020} as Lemma 3.5 in a more general setting, also covering a family of processes that are locally absolutely continuous to an $\alpha$-stable L\'evy process. 
...
% We will use a variation of a method introduced by Aldous in Proposition 15 of \cite{Aldous1997}. We use the description of this method provided in Subection 5.4 of \cite{conchon--kerjanStableGraphMetric2020}. 
...
% Firstly, by repeating the proof of Lemma 5.9 in \cite{conchon--kerjanStableGraphMetric2020}, we can show that for $\tilde{Y}^n_k=\zeta^n(\varepsilon^n_k)$, $\tilde{\Sigma}^n_k=\sigma^n(k-1)$, $$\tilde{\Xi}^n=\{(\tilde{Y}^n_k,\tilde{\Sigma}^n_k),k\geq 1\}$$ and $$\tilde{\Xi}^\infty=\{(\sigma_{l-},\zeta(\varepsilon^{(l)})):l\geq 0,\sigma_l-\sigma_{l-}>0\},$$
