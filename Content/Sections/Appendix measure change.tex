...
The $\P(\Delta_n = 0)$ divisor in the definition of $\phi_m^n$ can be handled by the standard local limit theorem.
\begin{lemma}
    \label{lem:normal-llt}
    As $n \to \infty$
    \begin{equation*}
        \P(\Delta_n = 0) = \frac{1}{\sqrt{2 \pi \sigma^2 n}} \left( 1 + \littleo(1) \right).
    \end{equation*}
\end{lemma}
\begin{proof}
...
\begin{proof}[Proof of \cref{lem:measure-change-approx}]
    ...
    \begin{align*}
        &\phi^n_m(\vk_1, \ldots, \vk_m) \\
        =& \frac{1}{\P(\Delta_n = 0)} \E \left[\prod_{i=1}^m \frac{(n-i+1)\mu}{\sum_{k=i}^m k^-_i + \Xi^-_{n-m}} \one_{A_n} \right] \\
        \geq& \frac{1}{\P(\Delta_n = 0)} \E_n \left[ \exp \left(
        ...
        \geq& \exp \left(
            ...
            \right) (1 + \littleo(1)) \frac{\P_n(\omegaevent_n \cap A_n)}{\P(\Delta_n = 0)}.
    \end{align*}
    ...
    \begin{equation*}
        \frac{\P_n(\omegaevent_n \cap A_n)}{\P(\Delta_n = 0)} \geq 1 + \littleo(1)
    \end{equation*}
...
\begin{lemma}
    Suppose that $(X_n)_{n \geq 0}$ and $(Y_n)_{n \geq 0}$ are a sequence of non-negative random variables such that $X_n \geq Y_n$ almost surely for all $n$. Further suppose $\E[X_n] = 1$ for all $n$ and that there exists $X$ such that $Y_n \to X$ almost surely as $n \to \infty$ and $\E[X] = 1$. Then $X_n \to X$ almost surely also, moreover $(X_n)_{n \geq 1}$ is a uniformly integrable sequence.
\end{lemma}