...
In order to sample a graph with a given degree sequence uniformly, we first consider the \emph{directed configuration model} as considered by \citet{cooperSizeLargestStrongly2004}. Take $n$ vertices $v_1, \ldots, v_n$ such that $v_i$ has $d^-(i)$ in-half-edges and $d^+(i)$ out-half-edges. Then construct a multigraph by choosing a uniform random pairing of the in-half-edges with the out-half-edges. The resulting random multigraph is the directed configuration model. \citet{cooperSizeLargestStrongly2004} then proved that if we condition on the directed configuration model being simple, we obtain a uniformly chosen random graph with the given degree sequence.
...
\end{align*}
We can obtain an identical expression for $\E[d^-(V)]$. \citet{cooperSizeLargestStrongly2004} show that, under additional assumptions, there exists a phase transition in the existence of a giant SCC depending on if the above quantity if strictly greater than or less than 1. Returning to the random degree sequence and, for now, ignoring conditioning on equal in- and out-degrees we have
\begin{equation*}
    \frac{\sum_{i=1}^n D_i^+ D_i^-}{\sum_{i=1}^n D_j^{-}}
    \to \frac{\E[D^- D^+]}{\E[D^-]}
\end{equation*}
...
\begin{corollary}
Consider $\vec{G}_n(\nu)$, with $\nu$ such that $$\mu=\sigma_+=\frac{\sigma_{-+}+\nu_-}{\mu}=1.$$ (Note that this condition is satisfied by $\nu(k^-,k^+)=\nu_1(k^-)\nu_2(k^+)$, with $\nu_i$ the law of a $\operatorname{Poisson}(1)$ random variable.)
