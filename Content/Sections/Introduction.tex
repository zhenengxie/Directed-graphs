...
\begin{enumerate}
    ...
    \item We visit the dummy leaves in $(\hat{F}_n(k),k\geq 1)$ in depth-first order, and for each vertex we sample whether it corresponds to a candidate.
...
\begin{enumerate}
    ...
    \item We identify the tails and heads of the candidates, and recover the SCCs from the resulting digraph via a cutting procedure. We use a result from \cite{goldschmidtScalingLimitCritical2019} to show that the cutting procedure converges. This summarised in Corollary \ref{cor.sccordereduptotimeT}.
