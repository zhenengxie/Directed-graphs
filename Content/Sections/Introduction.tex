...
The fourth assumption is the criticality condition. To understand how this arises, consider the directed configuration model and let $(V_n, W_n)$ be a uniformly chosen edge. First condition on the degree sequence $(\vD_1, \ldots, \vD_n)$. Since we choose an edge pairing uniformly at random, $W = i$ with probability proportional to $D^-_i$. Thus, the expected out-degree of $W_n$ is given by
\begin{align*}
    \E[d^+(W_n) \mid \vD_1, \ldots, \vD_n]
...
\end{equation*}
Let $(Z^-, Z^+)$ follow this distribution, that is $P(Z^- = k^-, Z^+ = k^+)$ is given by the above expression. The results of Cooper and Frieze then suggest the criticality condition to be $\E[Z^+] = 1$. This is equivalent to $\E[D^- D^+] = \E[D^-]$. Our work in the sequel confirms that this is the case.
