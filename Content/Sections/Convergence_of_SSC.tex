...
\end{proposition}
\begin{proof}
By Skorokhod's representation theorem, we may work on a probability space where the convergence in Proposition \ref{prop.convergenceancestraledges} holds almost surely, and claim that we can apply Lemma \ref{lemma.extractexcursions} to the sample paths of $\left(n^{-1/3}\hat{S}^{+}_n\left(\lfloor n^{2/3}t\rfloor\right),t \leq T+T'\right)$ with marks $$\left(n^{-2/3}X_n^i\right)_{1\leq i\leq A_n\left(\lfloor T n^{2/3}\rfloor\right)},$$ where $T'$ is large enough such that the tree visited at time $T$ has fewer than $T'n^2/3$ vertices with high probability; such a $T'$ exists by the convergence under rescaling of the \L ukasiewicz path of the out-forest. We check the conditions.
...
Finally, we claim that we can choose $T$ large enough such that the largest SCCs are explored before time $\lfloor Tn^{2/3}\rfloor$. This is the content of the following lemma. The proof is in the same spirit as \citet[Lemma 9]{aldous_1991}.
