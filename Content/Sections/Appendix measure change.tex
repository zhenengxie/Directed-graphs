...
\begin{lemma}
    ...
    \begin{align*}
        &\left( 
            \Phi(n, \floor{T n^{2/3}}),
            \left(
                n^{-1/3} S^-\left( \floor{n^{2/3} t} \right),
                n^{-1/3} S^+\left( \floor{n^{2/3} t} \right)
            \right)_{t \in [0, T]}
        \right) \\
...
\cref{lem:measure-change} then follows as a direct corollary of \cref{lem:measure-change-no-crit} using the continuous mapping theorem.
...
\begin{proof}
    ...
    In particular $X_n - Y_n \todist 0$ a constant and hence can be included in the joint convergence statement. This means
    \begin{equation*}
    ...
    We now prove the uniform integrability. By Skorokhod's representation theorem we can suppose that $X_n \to Y$ almost surely. Then $\E[X_n] = \E[Y] = 1$ for all $n$. Therefore $X_n \to Y$ in $L^1$ by Scheffe's lemma. This gives the required uniform integrability.
\end{proof}
...
\begin{proof}[Proof of \cref{lem:measure-change-no-crit}]
    ...
    \end{equation*}
    The processes $(S^{\pm}(n))_{n \geq 0}$ are discrete martingales. Therefore by Doob's maximal inequality
    \begin{equation*}
    ...
    \begin{align*}
        &\left( 
            \Phi(n, \floor{T n^{2/3}}) ,
            \left(n^{-1/3} S^-(\floor{t n^{2/3}}), n^{-1/3} S^+(\floor{t n^{2/3}}) \right)_{t \in [0, T]}
         \right) \\
        & \hspace{22em} \todist
         \left( 
             \Phi(T), (\sigma_- W^-_t, \sigma_+ W^+_t)_{t \in [0, T]}
